...
# Config
# ============================================================
DATA_DIR = "/home/paramjeet/times-fm/datasets/SMD"
OUTPUT_DIR = "/home/paramjeet/times-fm/experiments/SMD_results"
...
TRAIN_FILE = "SMD_machine-1-2_train.csv"
TEST_FILE  = "SMD_machine-1-2_test.csv"

CONTEXT_LIST = [256,512]
HORIZON_LIST = [1,10,20]
...
for context_len in CONTEXT_LIST:
    for horizon in HORIZON_LIST:
        ...
        config_name = f"SMD_context{context_len}_h{horizon}"

        # ================= SAVE SCORES =================
        np.save(
            os.path.join(CHECKPOINT_DIR, f"smd_{config_name}_scores.npy"),
            anomaly_scores
        )
        ...
        roc_df.to_csv(
            os.path.join(CHECKPOINT_DIR, f"smd_{config_name}_roc_values.csv"),
            index=False
        )
        ...
        pr_df.to_csv(
            os.path.join(CHECKPOINT_DIR, f"smd_{config_name}_pr_values.csv"),
            index=False
        )
        ...
        plt.legend()
        plt.savefig(os.path.join(PLOT_DIR, f"smd_{config_name}_roc.png"))
        plt.close()
        ...
        plt.legend()
        plt.savefig(os.path.join(PLOT_DIR, f"smd_{config_name}_pr.png"))
        plt.close()

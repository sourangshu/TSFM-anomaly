...
OUTPUT_DIR = "/home/paramjeet/times-fm/experiments/chronos/exathlon_256_1_chronos_31_ts_"
os.makedirs(OUTPUT_DIR, exist_ok=True)

log_file = os.path.join(OUTPUT_DIR, "eexathlon_256_1_chronos_31_ts_run.log")
logging.basicConfig(
    level=logging.INFO,
...
summary_csv = os.path.join(OUTPUT_DIR, "Exathlon_summary.csv")
summary_df.to_csv(summary_csv, index=False)
logger.info(f"Saved per-series summary to {summary_csv}")

# Compute mean metrics
mean_auroc = summary_df["AUROC"].mean(skipna=True)
mean_auprc = summary_df["AUPRC"].mean(skipna=True)
...
mean_csv = os.path.join(OUTPUT_DIR, "Exathlon_mean_metrics.csv")

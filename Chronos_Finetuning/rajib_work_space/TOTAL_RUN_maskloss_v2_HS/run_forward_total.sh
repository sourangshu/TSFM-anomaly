...
#
# Inference is IDENTICAL across all methods by construction: forward.py is byte-identical
# in every arm (md5 a226d1f5e899d7ae332112e3f29d076f), the test pkls here are symlinks to
# the very bytes TOTAL_RUN_maskloss_v2 evaluated on, and the only line that differs from
# that arm's copy of this script is the default CHECKPOINT path.
#

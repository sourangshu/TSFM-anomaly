...
# with a UNIFIED (global) per-dataset normal prefix (Unified_Normal_Total_run_HS).
#
# TRAINING IS BYTE-IDENTICAL to ../TOTAL_RUN_maskloss_v2_HS: same
# finetune_anomaly_simple.py (md5-matched copy), same sampler, same loss, same
# hyperparameters. The ONLY difference in this arm lives in the DATA — this
# folder's prepare_total.py attaches a UNIFIED per-dataset normal prefix (the
# dataset's training-file medoid, re-scaled per series) instead of a per-series
# one. Run ./run_prepare_total.sh here first.
#
...
#
# Relative to TOTAL_RUN_maskloss_v2, the HS arm changed the sampler; this arm keeps
# that sampler and changes only the normal prefix in the prepared data.
...
# This script lives in rajib_work_space/Unified_Normal_Total_run_HS. The training
...
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/chronos2-single-stage_mtsbench_maskLossv2_HS_UN_v1}"
...
BATCH_SIZE="${BATCH_SIZE:-80}"                 # CHANNEL ROWS, not windows
...
echo "======================================================"
echo "  Chronos-2 Single-Stage Anomaly Fine-Tuning  [HS + UNIFIED NORMAL]"
echo "======================================================"

...
The VALIDATION set comes from mTSBench's own *val.csv files — one per dataset,
never touched by the train/test *test.csv split — and is carved HIERARCHICALLY so
that every trained dataset is represented equally, mirroring the sampler:

    level 1   EQUAL budget (--val_per_dataset) for every dataset in the train pool
    level 2   an anomalous share of --val_p_anom (=1/3) inside each dataset
    level 3   within the anomalous kind, windows are drawn count-weighted by n_anom
              (the normal kind has n_anom=0 throughout, so it is drawn uniformly)

drawn WITHOUT replacement under a per-dataset seed, so the val set is a fixed,
deterministic pkl. Level 2 is best-effort: a dataset that owns fewer than
p_anom * budget anomaly windows (GutenTAG, SMAP, SMD have single digits) hands
over every one it has and tops the budget up with normal windows, so the EQUAL
per-dataset count always holds and the realized mix is recorded in the manifest.
Datasets with no train half are absent from val exactly as they are absent from
level 1.

  NOTE — the val slice is unseen but not recording-disjoint: mTSBench's X_val.csv
  is a different time range of the SAME recording as X_test.csv, which may sit in
  either half of our split. No val timestep is ever trained on; the recording may
  be. Model selection on eval_val_loss is therefore mildly optimistic in absolute
  terms but stays comparable across arms (identical val bytes by construction).
...
Products:
  ...
  3. val_model_inputs.pkl  (flat, dataset-tagged, hierarchically balanced — see above)
  4. manifest.json  (adds unified_reference / unified_mean_similarity per dataset)
...
-----
    ...
    python prepare_total.py --val_only                       # add val to an existing
                                                             # prepared_total (train/test
                                                             # pkls left untouched)
"""
...
def build_val_windows(ds, ddir, args, min_req, global_norm):
    ...
    *val.csv or the file is too short to yield a single window."""
    ...
    val_pairs, _, _, _ = build_pairs_for_files(
        ...
        args.val_stride, min_req, f"{ds}/val", global_norm=global_norm)
...
def prepare_val_only(args) -> None:
    ...
    The train/test halves are exactly what they were — this only reads each dataset's
    already-saved medoid (global_normal_signal.npz) so the val windows get the same
    UNIFIED prefix, then writes val_model_inputs.pkl and patches the manifest. Safe to
    run against a prepared_total a fine-tune is already reading: no train or test pkl
    ...
    for ds in train_datasets:
        logger.info("=" * 78)
        logger.info(f"{ds}")
        npz = os.path.join(per_ds_root, ds, "global_normal_signal.npz")
        if not os.path.exists(npz):
            raise SystemExit(
                f"{npz} missing — the val windows must carry the SAME unified prefix as "
                f"this dataset's train windows, and it is only stored there. Re-run the "
                f"full prep for {ds}."
            )
        global_norm = np.load(npz)["signal"].astype(np.float32)

        sel, vstats = build_val_windows(ds, os.path.join(args.data_root, ds),
                                        args, min_req, global_norm)
...
def prepare(args) -> None:
    ...
    for ds in datasets:
        ...
        if train_files:
            ...
            # ── VAL half — mTSBench's own *val.csv, same UNIFIED prefix ──────
            ...
            # (and no leakage-free medoid) to track.
            if not args.no_val:
                sel, vstats = build_val_windows(ds, ddir, args, min_req, global_norm)
                if vstats is not None:
                    val_pool.extend(sel)
                    val_datasets.append(ds)
                    entry.update(vstats)
...
def main():
    ...
    p.add_argument("--val_only", action="store_true",
                   help="Add ONLY the val set to an existing prepared_total, reusing each "
                        "dataset's saved medoid. Train/test pkls are never rewritten, so "
                        "this is safe to run against data a fine-tune is already reading.")

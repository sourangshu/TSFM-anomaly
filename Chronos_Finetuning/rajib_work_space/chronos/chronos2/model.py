...
class Chronos2Model(PreTrainedModel):
    ...
    def __init__(self, config: Chronos2CoreConfig):
        ...
        # Token vocabulary: [PAD]=0, then optional [REG] and [SEP]
        vocab_size = 1  # PAD
        if self.chronos_config.use_reg_token:
            config.reg_token_id = vocab_size
            vocab_size += 1
        if self.chronos_config.use_sep_token:
            config.sep_token_id = vocab_size
            vocab_size += 1
        ...
        # print(f"++++++++++++++input patch embedding shape: {self.input_patch_embedding}++++++++++++++")
        # patching layer
        self.patch = Patch(
        ...
        # print(f"++++++++++++++output patch embedding shape: {self.output_patch_embedding}++++++++++++++")
    ...
    def _prepare_patched_context(
        ...
        # print(f"------------------patched_context shape after patching: {patched_context.shape}------------------")
        ...
        # print(f"___________________{context_time_enc.shape}___________________")
        # print(f"___________________{patched_context.shape}___________________")
        # print(f"___________________{patched_mask.shape}___________________")
    ...
    def forward(
        ...
        """
        batch_size = context.shape[0]
        # print(f"--------group_ids: {group_ids}--------before encode")
        ...
        assert hidden_states.shape == (batch_size, num_context_patches + num_special_tokens + num_output_patches, self.model_dim)

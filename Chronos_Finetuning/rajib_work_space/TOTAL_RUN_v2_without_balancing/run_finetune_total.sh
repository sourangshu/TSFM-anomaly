...
#
# This is a 1:1 copy of the SMD_run/run_finetune_smd.sh configuration that gave the
# good SMD results (per-series normal prefix, margin/hinge loss, LoRA, SEP token) —
# only PREPARED_DIR and OUTPUT_DIR differ. The combined train pkl mixes datasets
# with DIFFERENT feature counts (2..72); Chronos2Dataset flattens each window into
# per-variate rows tagged by group_ids, so variable-variate windows batch fine.
#
# The combined train pkl has NO validation split (NO_VALIDATION=1), exactly like SMD.
#
# Usage:
#   bash run_finetune_total.sh
#   NUM_STEPS=8000 bash run_finetune_total.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/chronos2-single-stage_TOTAL_v2}"
TEST_DATA="${TEST_DATA:-}"
...
# Training hyperparameters (match SMD_run/run_finetune_smd.sh)
...
echo "======================================================"
echo "  Chronos-2 Single-Stage Fine-Tuning — TOTAL v2 (mTSBench, NO balancing)"
echo "======================================================"
...
echo "  LR                = $LR   SCHED=$LR_SCHEDULER   WARMUP=$WARMUP_RATIO"
...
if [ ! -f "${PREPARED_DIR}/train_model_inputs.pkl" ]; then
    echo "ERROR: ${PREPARED_DIR}/train_model_inputs.pkl not found. Run ./run_prepare_total.sh first."; exit 1
fi

...
class Chronos2Pipeline(BaseChronosPipeline):
    ...
    def fit(
        ...
        **extra_trainer_kwargs,
        ...
        enable_sep_token
            If True, enables the [SEP] token in the new model. The input sequence
            becomes [normal patches] [SEP] [context patches] [REG] [future patches].
            When enabling on a pretrained checkpoint that did not have SEP, the shared
            embedding table is expanded by one row (random init for the new SEP token).
            The caller is responsible for laying out the `target` array so that the
            first `sep_patch_index * input_patch_size` timesteps are the normal signal,
            and the remaining timesteps up to `context_length` are the actual context.
        sep_patch_index
            Patch index at which to insert the [SEP] token. Only used when `enable_sep_token=True`.
            Example: with `input_patch_size=16` and `normal_signal_length=512`, set
            `sep_patch_index=32` (= 512 / 16).
        **extra_trainer_kwargs
        ...
        # Optionally enable [SEP] token on the new model. When enabling SEP
        # on a checkpoint that did not have it, the shared embedding table grows
        # by one row, which is copied below from the old weights + random init for SEP.
        if enable_sep_token and not config.chronos_config.get("use_sep_token", False):
            ...
            logger.info(
                f"Enabling [SEP] token at patch index {sep_patch_index}. "
                "Shared embedding table will be expanded by one row (random init for SEP)."
            )
        elif config.chronos_config.get("use_sep_token", False):
            # already enabled in the loaded config; allow caller to override sep_patch_index
            if enable_sep_token:
                config.chronos_config["sep_patch_index"] = sep_patch_index

        model = Chronos2Model(config).to(self.model.device)  # type: ignore

        # Load state_dict with vocab expansion handling: if the new model has a
        # larger `shared` embedding (e.g., we just enabled SEP), copy the old
        # rows into the new table and leave the new row(s) randomly initialized.
        old_state = self.model.state_dict()
        new_shared = model.shared.weight.data
        old_shared = old_state.get("shared.weight", None)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        if find_adapter_config_file(pretrained_model_name_or_path) is not None:
            ...
            # Detect vocab size mismatch: adapter may have been trained with an expanded
            # shared embedding (e.g., a [SEP] token was added). If the adapter checkpoint
            # contains shared.weight with more rows than the base model's vocab_size, we
            # must build the base model with the expanded vocab before applying the adapter.
            import json as _json
            from pathlib import Path as _Path

            _ckpt_path = _Path(pretrained_model_name_or_path)
            _adapter_cfg_file = find_adapter_config_file(pretrained_model_name_or_path)
            _safetensors_path = _ckpt_path / "adapter_model.safetensors"
            _needs_vocab_expand = False
            _base_model_name = None
            _adapter_vocab_size = None

            if _adapter_cfg_file is not None and _safetensors_path.exists():
                try:
                    import safetensors.torch as _st

                    with open(_adapter_cfg_file) as _f:
                        _adapter_cfg = _json.load(_f)
                    _base_model_name = _adapter_cfg.get("base_model_name_or_path")

                    _adapter_state = _st.load_file(str(_safetensors_path))
                    _adapter_shared = _adapter_state.get("base_model.model.shared.weight")

                    if _adapter_shared is not None and _base_model_name is not None:
                        _base_config = AutoConfig.from_pretrained(_base_model_name)
                        _base_vocab_size = getattr(_base_config, "vocab_size", None)
                        _adapter_vocab_size = _adapter_shared.shape[0]
                        if _base_vocab_size is not None and _adapter_vocab_size != _base_vocab_size:
                            _needs_vocab_expand = True
                            logger.info(
                                f"Adapter shared.weight vocab size ({_adapter_vocab_size}) differs from "
                                f"base model vocab size ({_base_vocab_size}). "
                                "Loading base model with expanded vocab before applying PEFT adapter."
                            )
                except Exception as _e:
                    logger.warning(f"Could not pre-check vocab size for PEFT adapter ({_e}); falling back to standard load.")

            if _needs_vocab_expand:
                # Build an expanded base model: start from the base config, bump vocab_size,
                # copy existing base-model weights row-by-row, leave extra rows as random init.
                _base_config_expanded = AutoConfig.from_pretrained(_base_model_name)
                _n_extra = _adapter_vocab_size - _base_config_expanded.vocab_size
                if _n_extra >= 1 and hasattr(_base_config_expanded, "chronos_config"):
                    _base_config_expanded.chronos_config["use_sep_token"] = True
                _base_config_expanded.vocab_size = _adapter_vocab_size

                _base_model = Chronos2Model(_base_config_expanded)
                _base_model_orig = Chronos2Model.from_pretrained(_base_model_name)
                _orig_state = _base_model_orig.state_dict()
                _orig_shared = _orig_state["shared.weight"]
                _new_shared = _base_model.shared.weight.data.clone()
                _new_shared[: _orig_shared.shape[0]] = _orig_shared.to(_new_shared.dtype)
                _orig_state["shared.weight"] = _new_shared
                _base_model.load_state_dict(_orig_state)
                del _base_model_orig

                # Move to the requested device before handing to PeftModel
                _device_map = kwargs.get("device_map", None)
                if _device_map is not None and _device_map not in ("auto", "balanced", "balanced_low_0", "sequential"):
                    _base_model = _base_model.to(_device_map)

                peft_kwargs = {k: v for k, v in kwargs.items() if k != "device_map"}
                model = PeftModel.from_pretrained(_base_model, str(_ckpt_path), *args, **peft_kwargs)
